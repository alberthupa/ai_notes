# INTRO
    - ani nie mam wyksztalcenia informatycznego ani nie pracowalem w firmie IT
    - ale jestem swirem od danych i mocno sie wkrecilem w rozwiazania IT w obecnej firmie
    - obecnie najbardziej jestem wkrecony w routery do zarzadzania konwersacja

    - bycie architektem rozwiązań AI
    - W LLM chodzi o to zeby polaczyc FineTune + zarządzanie rozmową
 

# TODO
    - model garden
    - RNN na pytorch
    - transformers api na hugginface
    - czy mozna puszczac modele na hugginface?
...
- chaty:
    - opisy produktów
    - PDF agent (relacje inwestorskie) - RAG na FAISS jako kontekst do system prompt + finetuning (RL) na GPT3 (kompresja kontekstów w czasie) + propozycja komentarza 
    - SQL agent do bazy klubowiczów z AI ready model + kontekstowe wstrzykiwanie schemy + wizualizacja
    - SEO tekst generator - crawler + inspiracje na FAISS + API Senuto + management kroków
- Vertex AI: 
    - predykcja kto kupi
    - rekomendacje do apki (pytorch - lepiej niż biznes / gorzej niż most popular
- dom:
    - sciagnie modeli na LLM STudio
    - autogen
    - chain manager -> one-step / chain of thoughts / branches / tree of thoughts / 
